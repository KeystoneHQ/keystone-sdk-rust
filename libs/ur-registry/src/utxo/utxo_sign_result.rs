use alloc::string::ToString;
use alloc::vec::Vec;
use minicbor::encode::Write;
use minicbor::{Decoder, Encoder};
use minicbor::data::{Int};
use crate::cbor::cbor_map;
use crate::error::{URError, URResult};
use crate::registry_types::{RegistryType, UTXO_SIGN_RESULT};
use crate::traits::{RegistryItem, To, From as FromCbor};
use crate::types::Bytes;

const SIGN_RESULT: u8 = 1;

#[derive(Clone, Debug, Default)]
pub struct UtxoSignResult {
    sign_result: Bytes,
}

impl UtxoSignResult {
    pub fn default() -> Self {
        Default::default()
    }

    pub fn set_sign_result(&mut self, signature: Bytes) {
        self.sign_result = signature;
    }

    pub fn new(signature: Bytes) -> Self {
        UtxoSignResult { sign_result: signature }
    }

    pub fn get_sign_result(&self) -> Bytes {
        self.sign_result.clone()
    }
}

impl RegistryItem for UtxoSignResult {
    fn get_registry_type() -> RegistryType<'static> {
        UTXO_SIGN_RESULT
    }
}

impl<C> minicbor::Encode<C> for UtxoSignResult {
    fn encode<W: Write>(&self,
                        e: &mut Encoder<W>,
                        _ctx: &mut C) -> Result<(), minicbor::encode::Error<W::Error>> {
        e.map(1)?;
        e.int(Int::from(SIGN_RESULT))?
            .bytes(&self.sign_result)?;
        Ok(())
    }
}


impl<'b, C> minicbor::Decode<'b, C> for UtxoSignResult {
    fn decode(d: &mut Decoder<'b>, _ctx: &mut C) -> Result<Self, minicbor::decode::Error> {
        let mut result = UtxoSignResult::default();
        cbor_map(d, &mut result, |key, obj, d| {
            let key = u8::try_from(key).map_err(|e| minicbor::decode::Error::message(e.to_string()))?;
            match key {
                SIGN_RESULT => {
                    obj.sign_result = d.bytes()?.to_vec();
                }
                _ => {}
            }
            Ok(())
        })?;
        Ok(result)
    }
}

impl To for UtxoSignResult {
    fn to_bytes(&self) -> URResult<Vec<u8>> {
        minicbor::to_vec(self.clone()).map_err(|e| URError::CborEncodeError(e.to_string()))
    }
}

impl FromCbor<UtxoSignResult> for UtxoSignResult {
    fn from_cbor(bytes: Vec<u8>) -> URResult<UtxoSignResult> {
        minicbor::decode(&bytes).map_err(|e| URError::CborDecodeError(e.to_string()))
    }
}

#[cfg(test)]
mod tests {
    use alloc::vec::Vec;
    use crate::traits::{From as FromCbor, To};
    use hex::FromHex;
    use crate::utxo::utxo_sign_result::UtxoSignResult;

    #[test]
    fn test_encode() {
        let sign_result = hex::decode("").unwrap();
        let utxo_signature = UtxoSignResult::new(sign_result);
        assert_eq!(
            "",
            hex::encode(utxo_signature.to_bytes().unwrap()).to_lowercase()
        );
    }

    #[test]
    fn test_decode() {
        let bytes = Vec::from_hex(
            "",
        ).unwrap();

        let utxo_sign_result = UtxoSignResult::from_cbor(bytes).unwrap();
        let expect_result = hex::decode("").unwrap().to_vec();
        assert_eq!(expect_result, utxo_sign_result.get_sign_result());
    }
}
