...
use crate::registry_types::{RegistryType, ICP_SIGNATURE, UUID};
...
mod tests {
    ...
    use crate::{icp::icp_signature::IcpSignature, solana::sol_signature::SolSignature};
    use alloc::vec::Vec;
    use hex::FromHex;
    ...
    fn test_encode() {
        ...
        let icp_signature = IcpSignature::new(request_id, signature);
        assert_eq!(
    ...
    fn test_decode() {
        let bytes = Vec::from_hex(
            ...
            .unwrap();
        let icp_signature = SolSignature::from_cbor(bytes).unwrap();
        assert_eq!(
            [155, 29, 235, 77, 59, 125, 75, 173, 155, 221, 43, 13, 123, 61, 203, 109].to_vec(),
            icp_signature.get_request_id().unwrap()
        );
