...
impl AptosSignature {
    ...
    pub fn new(request_id: Bytes, signature: Bytes, public_key: Bytes) -> Self {
        AptosSignature { request_id, signature, authentication_public_key: public_key }
    }
...
impl <C> minicbor::Encode<C> for AptosSignature {
    fn encode<W: minicbor::encode::Write>(&self, e: &mut minicbor::Encoder<W>, _ctx: &mut C) -> Result<(), minicbor::encode::Error<W::Error>> {
        ...
        e.int(Int::try_from(AUTHENTICATION_PUBLIC_KEY).map_err(|e| minicbor::encode::Error::message(e.to_string()))?)?
